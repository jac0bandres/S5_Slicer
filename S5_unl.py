...
def _tetgen_worker(points, faces, tg_kwargs, out_path):
    """Tetrahedralize in a child process and pickle the result to `out_path`.

    TetGen does not merely raise on pathological surfaces (self-intersections,
    degenerate/near-coplanar facets) — it can *segfault*, an uncatchable hard
    crash that would otherwise take down the whole slicer.  Running it in a
    forked child contains the crash: the child dies, `out_path` is never
    written, and the parent treats that as a (recoverable) failure.
    """
    import pyvista as pv
    import tetgen
    import pickle
    ...
    try:
        ...
        t.tetrahedralize(**tg_kwargs)
        with open(out_path, "wb") as fh:
            pickle.dump({"ok": True, "grid": t.grid}, fh)
...
def slice(
    ...
    def _run(s, tg_kwargs):
        """Tetrahedralize `s` in an isolated subprocess and return the pv grid.

        Raises RuntimeError on either a TetGen error *or* a hard crash
        ...
        proc = mp.get_context("fork").Process(
            target=_tetgen_worker,
            args=(np.asarray(s.points, float), np.asarray(s.faces), tg_kwargs, out),
        )
        proc.start()
        proc.join()
        if not os.path.exists(out):
            raise RuntimeError(
                f"TetGen crashed (exit code {proc.exitcode}) — unrecoverable "
    ...
    PLAIN = {"maxvolume": max_volume} if max_volume else {}
    tiers = [
        ("as-is", lambda: _run(surf, QUAL)),
        ("repaired", lambda: _run(_repaired(), QUAL)),
        ("repaired + loose epsilon", lambda: _run(_repaired(), {**QUAL, "epsilon": 1e-4})),
        ("repaired + plain CDT", lambda: _run(_repaired(), {**PLAIN, "epsilon": 1e-4})),
    ]
    ...
    for name, attempt in tiers:
        try:
            input_tet = attempt()
            if name != "as-is":
                print(f"  TetGen succeeded with tier: {name}")
            break
        except RuntimeError as e:
            print(f"  TetGen [{name}] failed: {str(e).strip()}")
        finally:
            _cleanup_tetgen_scratch()

    if input_tet is None:
        raise RuntimeError(
            "TetGen could not tetrahedralize the mesh after repair and all "
            "geometry-preserving fallbacks. The surface almost certainly has "
            "self-intersections that pymeshfix cannot resolve (common on "
            "lattice / frame models). Resolve them upstream in CAD, or with an "
            "exact tool (pymeshlab / CGAL corefinement) before slicing."
        )
